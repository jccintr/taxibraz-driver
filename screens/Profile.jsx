import { StyleSheet,ToastAndroid,SafeAreaView,ScrollView,View,Text,TouchableOpacity,ActivityIndicator } from 'react-native';
...
const Profile = () => {
  ...
  const uploadAvatar = async (imageUri) => {
    ...
    const fileName = 'drivers/' + loggedUser._id + '.jpg'; //new Date().getTime() + '.jpg';
  ...
  const updateAvatar = async () => {
    ...
    if(formData.name.trim().length > 0 && formData.telefone.trim().length > 0){
       const response = await api.updateDriver(apiToken,formData);
...
const updateProfile = async () => {
  ...
  if(formData.name.trim().length === 0 || formData.telefone.trim().length === 0){
      alert("Por favor, informe o nome e o telefone.");
      return;
  }
  setIsLoading(true);
  const response = await api.updateDriver(apiToken,formData);
